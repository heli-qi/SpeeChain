   ...
   * The second-level keys are iterator names used for distinguishing the loaded data of each iterator. 
   ...
   Under the name of each iterator, there are two third-level keys whose names are fixed:
     ...
     The configuration is made up of the following 4 fourth-level keys:
         ...
         Please refer to [Dataset API Document]() for more details.
         ...
         There are 6 general arguments that can be set manually in _data_cfg:_ (please refer to [speechain.iterator.abs.Iterator.\_\_init\_\_]() for more details)  
            1. batches_per_epoch
            2. is_descending
            3. shuffle
            4. data_len
...
## API Document
1. [**speechain.iterator.abs.Iterator**]()  
   ...
   7. [batches_generate_fn]()
   
2. [**speechain.iterator.block.BlockIterator**]()
...
👆[Back to the API list](https://github.com/ahclab/SpeeChain#the-speechain-toolkit)
