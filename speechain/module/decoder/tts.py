...
class TTSDecoder(Module):
    ...
    def module_init(self,
        ...
        if spk_emb is not None:
            ...
            if 'd_model' in spk_emb.keys():
                assert spk_emb['d_model'] == _prev_output_size, \
                    "Your input d_model is different from the one generated by codes. " \
                    f"Your input one is {spk_emb['d_model']} but the generated one is {_prev_output_size}."
            else:
    ...
    def forward(self,
        ...
        if hasattr(self, 'spk_emb'):
            ...
            enc_text = self.spk_emb.combine_spk_emb(spk_feat=spk_feat, tgt=enc_text)
            # combine the speaker features with the decoder prenet outputs
