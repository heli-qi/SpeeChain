...
function print_help_message {
  ...
  $0 \\ (The arguments in [] are optional while other arguments must be given by your run.sh.)
    ...
    --subsets SUBSETS \\                   # A comma-separated string which defines the subsets of LibriTTS you want to download. (default: all subsets)
...
for (( n=0; n < ${#subsets[*]}; n++ )); do
  ...
  if [ ! -d "${download_path}/data/wav/${set}" ]; then
    ...
    if [ ! -f "${download_path}/data/${set}.tar.gz" ]; then
      echo "Download data from https://www.openslr.org/resources/60/${set}.tar.gz to ${download_path}/data/${set}.tar.gz"
      wget -P ${download_path}/data https://www.openslr.org/resources/60/"${set}".tar.gz
    else
      echo "${download_path}/data/${set}.tar.gz has already existed. Skipping data downloading~~~"
    fi

    # unzip the downloaded data package
    if [ ! -d "${download_path}/data/LibriTTS/${set}" ]; then
      ...
      echo "${download_path}/data/LibriTTS/${set} has already existed. Skipping data unzipping~~~"
    fi
...
# Finally, remove the folder named LibriTTS if needed
if [ -d "${download_path}/data/LibriTTS" ];then
  rm -rf "${download_path}/data/LibriTTS"
fi